...
const HeroSection: React.FC = () => {
  ...
  return (
    <section id="hero" className="relative bg-brand-dark-blue min-h-screen flex items-center justify-center overflow-hidden pt-20">
      <div className="absolute inset-0 opacity-20">
        {/* Subtle background pattern or effect if desired */}
        {/* Example: <div className="absolute inset-0 bg-grid-pattern opacity-10"></div> */}
      </div>
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-center">
          <div className={`text-center md:text-left transition-all duration-1000 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <p className="mt-6 text-lg sm:text-xl text-brand-secondary-text max-w-xl mx-auto md:mx-0">
          ...
          <div className={`relative mt-12 md:mt-0 transition-all duration-1000 ease-out delay-300 ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}>
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-brand-dark-blue via-transparent to-transparent opacity-50 rounded-xl"></div>
            <div className="absolute -bottom-4 -right-4 w-32 h-32 bg-brand-accent-purple opacity-20 rounded-full filter blur-2xl"></div>
            <div className="absolute -top-4 -left-4 w-32 h-32 bg-brand-accent-blue opacity-20 rounded-full filter blur-2xl"></div>
          </div>
        </div>
      </div>
    </section>
